/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.cpp                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jonascim <jonascim@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/29 09:54:39 by jonascim          #+#    #+#             */
/*   Updated: 2023/05/27 13:46:26 by jonascim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/Animal.hpp"
#include "../includes/Cat.hpp"
#include "../includes/Dog.hpp"

int main()
{
	int const arraySize = 10;
	Animal	*array[arraySize];
	// Animal test;
	// uncomment /\ to see the compile error related to abstract class

	for (int i = 0; i < arraySize; i++)
	{
		if (i % 2 == 0)
			array[i] = new Cat();
		else
			array[i] =  new Dog();
	}
	std::cout << "************************************************" <<  std::endl;
	std::cout << "************************************************" <<  std::endl;
	for (int j = 0; j < arraySize; j++)
	{
		std::cout << "Removing animal " << j + 1 << " from the array" << std::endl;
		delete array[j];
	}
	std::cout << "************************************************" <<  std::endl;
	std::cout << "************************************************" <<  std::endl;
	Dog *basic = new Dog();
	basic->getBrain().setIdea(1, "TEEEEST");
	basic->getBrain().setIdea(2, "TAAAAAATAAAA");
	basic->getBrain().setIdea(101, "NOPE");

	Dog tmp = *basic;
	std::cout << tmp.getType() << std::endl;
	std::cout << basic->getBrain().getIdea(1) <<  std::endl;
	std::cout << tmp.getBrain().getIdea(2) <<  std::endl;

	delete basic;

	std::cout << tmp.getBrain().getIdea(2) <<  std::endl;
	std::cout << std::endl;
	std::cout << "************************************************" <<  std::endl;
	std::cout << "************************************************" <<  std::endl;
	std::cout << std::endl;
	Cat *cat = new Cat();
	cat->makeSound();
	tmp.makeSound();
	std::cout << std::endl;
	delete cat;
	system("leaks animals");

	return (0);
}
